// LONGEST PALINDROME SUBSEQUENCE STRINGS--
//just take s2 as reverse of s1 as 
// if u want to print use lcsubsequence.cpp 
#include<bits/stdc++.h>
using namespace std;
//recursion O(2^n * 2^m),
...
int main(){
    ...
    cout << "LongestPalindormeSubsequence length = " << lcs(n-1, m-1, s1, s2) << endl;

    vector<vector<int>> dp(n,vector<int>(m,-1));
    cout << "LongestPalindormeSubsequence length = " << dplcs(n-1, m-1, s1, s2,dp) << endl;

    cout << "LongestPalindormeSubsequence length = " << tablcs(n, m, s1, s2) << endl;

    cout << "LongestPalindormeSubsequence length = " << space(n, m, s1, s2) << endl;
    return 0;
}
