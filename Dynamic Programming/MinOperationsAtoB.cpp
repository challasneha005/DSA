// MIN OPERATIONS TO GET A->B
// INSERTIONS /DELETIONS..
// deletion = n - len(lcs) #subsequnece
...
int main(){
    ...
    // total operations = n+m - 2*len(lcs)
    cout << "Min Operations = " <<  n+m - 2*(lcs(n-1, m-1, s1, s2)) << endl;

    vector<vector<int>> dp(n,vector<int>(m,-1));
    cout << "Min Operations = " << n+m - 2*(dplcs(n-1, m-1, s1, s2,dp)) << endl;

    cout << "Min Operations = " << n+m - 2*(tablcs(n, m, s1, s2)) << endl;

    cout << "Min Operations = " << n+m - 2*(space(n, m, s1, s2)) << endl;
    return 0;
}
